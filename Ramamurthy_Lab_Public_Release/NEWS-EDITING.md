...
Add each new `<article class="news-entry">` at the **top** of `.news-feed` in `index.html`, so the newest item appears first. The website shows the first three entries and puts the remaining entries behind **Show older news**. With three or fewer entries, that control stays hidden.
...
Save photos in `assets/news/` using filenames without spaces. Commit the edited `index.html` and new photo files to the publishing branch to update the site.
